...
export function processArmyHeader(lines) {
    const armyInfo = [];
...
export function formatUnitName(unitName, isTauEmpire, smartFormat) {
...
export function formatTauUnitName(unitName) {
...
export function processUnits(lines, startIndex, showPoints, smartFormat, isTauEmpire) {
    const cleanedLines = [];
    let currentUnit = '';
    let currentPoints = '';
    let lastUnit = '';
...
export function cleanRosterText(input, showPoints = true, smartFormat = true) {
    input = input.trim();
    if (!input) return '';
